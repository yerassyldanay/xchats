...
Откройте http://localhost:8081 и войдите под стандартной учётной записью
администратора:
...
После входа мастер первоначальной настройки проведёт вас через добавление
API-ключа LLM-провайдера, затем **Accounts → add**, чтобы подключить номер
WhatsApp по QR-коду, или **Settings → Integrations**, чтобы подключить
Telegram-бота.
