...
func NewVision(baseURL, apiKey, model string, maxTokens int) *Vision {
	if maxTokens <= 0 {
		maxTokens = 512
	}
	return &Vision{
...
// the caller falls back to a describe_media popup.
func (v *Vision) Describe(ctx context.Context, mimetype string, data []byte) (string, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return "", fmt.Errorf("vision: http %d: %s", resp.StatusCode, string(body))
	}
	var out struct {
		Choices []struct {
			Message struct {
	...
	if out.Error != nil {
		return "", fmt.Errorf("vision: %s", out.Error.Message)
	}
