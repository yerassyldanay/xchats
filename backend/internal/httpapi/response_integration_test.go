...
// drafting with the evaluated pipeline: a real inbound event is stored,
...
// left), and approving that draft delivers it through the fake WhatsApp
// sender — never a direct provider call from the response service itself.
func TestWhatsAppInboundProducesGroundedDraftAndApprovalDelivers(t *testing.T) {
	h := newHarnessWithLLM(t, groundedFakeLLMClient{})

	// 1. Inbound: a genuine customer message injected the same way a real
	// whatsmeow event (or /debug/wa-event) would be.
	h.inject(customerJID, "wa-grounded-1", "Сколько стоит кофемашина?", false)

	chats := h.listChats()
	if len(chats) != 1 {
		t.Fatalf("want 1 chat after the inbound event, got %d", len(chats))
	}
	...
	// sender registry's fake WhatsApp sender, since response.Service itself
	// has no route to any provider at all (see backend/response's dependency test).
	...
	calls := h.fake.TextCalls()
	if len(calls) != sendTextBefore+1 {
		t.Fatalf("approve should have sent exactly one text via the fake WhatsApp sender; before=%d after=%d", sendTextBefore, len(calls))
	}
...
// TestSimulatorApprovalNeverCallsWhatsApp is the full-stack complement to
...
// WhatsApp sender sitting in the very same messaging.SenderRegistry, which a
// channel-routing bug could easily send it to instead.
func TestSimulatorApprovalNeverCallsWhatsApp(t *testing.T) {
	h := newHarness(t)
	...
	if got := len(h.fake.Calls); got != callsBefore {
		t.Fatalf("simulator approval made %d fake WhatsApp call(s), want 0 (routed to the WhatsApp fake instead of the simulator sender)", got-callsBefore)
	}
