...
// accountOwner reads the owning organization (and the channel) of an account
// inside an inbound transaction. table is a package constant naming one of the
// three account families — wa_accounts, tg_accounts, channel_accounts — never
// caller input.
//
// tg_accounts is the one with no channel column: Telegram is its own table
// family precisely because it has exactly one channel. The other two carry the
// channel as data, which is what lets a new Meta channel reach the CRM layer
...
func accountOwner(ctx context.Context, tx *dbx.Tx, table string, accountID uuid.UUID) (uuid.UUID, string, error) {
	...
	case "channel_accounts":
		err = tx.QueryRow(ctx,
			`SELECT organization_id, channel FROM channel_accounts WHERE id = $1`, accountID).Scan(&org, &channel)
	default:
		err = tx.QueryRow(ctx,
