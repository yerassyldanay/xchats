...
// TestWriteDraftSet_InvalidStructuredKBGapNormalizedNotRejected is the P2
// finding's proof: DraftOption is exported, so aiprompt.sanitizeKBGap having
// already validated a MODEL-authored diagnostic is not a guarantee every
// caller of the store API went through it. A value outside the closed
// vocabulary must be normalized to the same safe default an absent one
// already gets — never silently persisted as-is (it would then be
// invisible to both Counts/OperationalCounts, which only recognize the
// closed set, while still showing up untranslated in Recent), and never
// rejected outright (an optional diagnostic must not fail the draft write
// it rides in on).
func TestWriteDraftSet_InvalidStructuredKBGapNormalizedNotRejected(t *testing.T) {
	st := dbtest.New(t)
	ctx := context.Background()
	_, _, chatID := seedOneChat(t, st)
	...
	_, err := st.WriteDraftSet(ctx, "whatsapp", chatID, uuid.NullUUID{}, []store.DraftOption{
		{
			Ordinal: 1, Text: "x", ReplyLanguage: "ru", Escalate: true,
			KBGapReasonCode: "totally_bogus_reason", KBGapSource: "totally_bogus_source",
			KBGapTargetEntityType: "totally_bogus_type", KBGapTargetEntityRef: "some-ref",
		},
	})
	if err != nil {
		t.Fatalf("WriteDraftSet: %v (an out-of-vocabulary diagnostic must never fail the write)", err)
	}
	...
	e := events[0]
	if e.ReasonCode != "other" {
		t.Errorf("ReasonCode = %q, want normalized to %q", e.ReasonCode, "other")
	}
	if e.Source != "model" {
		t.Errorf("Source = %q, want normalized to %q", e.Source, "model")
	}
	if e.TargetEntityType != "" || e.TargetEntityRef != "" {
		t.Errorf("target = %q/%q, want cleared (an invalid type must drop the ref too)", e.TargetEntityType, e.TargetEntityRef)
	}
}

// TestWriteDraftSet_TargetRefWithoutTypeClearsTarget covers the OTHER half
// of "both valid or neither": a ref with no type (or vice versa) is just as
// meaningless as an unrecognized type, so it must be cleared the same way.
func TestWriteDraftSet_TargetRefWithoutTypeClearsTarget(t *testing.T) {
	st := dbtest.New(t)
	ctx := context.Background()
	_, _, chatID := seedOneChat(t, st)
	...
	_, err := st.WriteDraftSet(ctx, "whatsapp", chatID, uuid.NullUUID{}, []store.DraftOption{
		{
			...
			KBGapTargetEntityRef: "coffee-machine", // type deliberately left empty
		},
	})
	if err != nil {
		t.Fatalf("WriteDraftSet: %v", err)
	...
	if e := events[0]; e.TargetEntityType != "" || e.TargetEntityRef != "" {
		t.Errorf("target = %q/%q, want cleared (a ref without a type is meaningless)", e.TargetEntityType, e.TargetEntityRef)
	}
}

// TestWriteDraftSet_DuplicateMissingFieldDoesNotRollBackDraft is the
// end-to-end proof for the P1 finding: ai_kb_gap_missing_fields has
// UNIQUE(event_id, field_name), so a DraftOption built directly with a
// repeated field (bypassing aiprompt.sanitizeKBGap's own dedupe — DraftOption
// is exported, so nothing guarantees every caller went through it) must
// still let the whole write succeed, draft included, rather than rolling
// back an otherwise-valid customer draft over an optional diagnostic.
func TestWriteDraftSet_DuplicateMissingFieldDoesNotRollBackDraft(t *testing.T) {
	st := dbtest.New(t)
	ctx := context.Background()
	_, _, chatID := seedOneChat(t, st)
	...
	drafts, err := st.WriteDraftSet(ctx, "whatsapp", chatID, uuid.NullUUID{}, []store.DraftOption{
		{
			...
			EscalationReason: "нет цены", KBGapReasonCode: "missing_field",
			KBGapTargetEntityType: "product", KBGapTargetEntityRef: "coffee-machine",
			KBGapMissingFields: []string{"price", "price", "warranty_terms"},
		},
	})
	if err != nil {
		t.Fatalf("WriteDraftSet: %v (a duplicate missing_fields entry must never fail the write)", err)
	}
	if len(drafts) != 1 {
		t.Fatalf("want 1 draft, got %d", len(drafts))
	}
	...
	wantFields := map[string]bool{"price": true, "warranty_terms": true}
	got := events[0].MissingFields
	if len(got) != 2 || !wantFields[got[0]] || !wantFields[got[1]] {
		t.Errorf("MissingFields = %v, want [price warranty_terms] deduped", got)
	}
}
...
func seedGapEventWithTarget(t *testing.T, st *store.Store, chatID uuid.UUID, entityType, entityRef string, fields []string) {
	t.Helper()
	if _, err := st.WriteDraftSet(context.Background(), "whatsapp", chatID, uuid.NullUUID{}, []store.DraftOption{
		{
			Ordinal: 1, Text: "x", ReplyLanguage: "ru", Escalate: true, KBGapReasonCode: "missing_field",
			KBGapTargetEntityType: entityType, KBGapTargetEntityRef: entityRef, KBGapMissingFields: fields,
		},
	}); err != nil {
		t.Fatalf("seedGapEventWithTarget(%s/%s): %v", entityType, entityRef, err)
	}
}

func entityCountFor(counts []store.KBGapTargetEntityCount, entityType, ref string) int {
	for _, c := range counts {
		if c.TargetEntityType == entityType && c.TargetEntityRef == ref {
			return c.Count
		}
	}
	return -1
...
func fieldCountFor(counts []store.KBGapMissingFieldCount, entityType, field string) int {
	for _, c := range counts {
		if c.TargetEntityType == entityType && c.FieldName == field {
			return c.Count
		}
	}
	return -1
...
func TestKBGapReportFor_DefaultReportIsZeroFilledAndSplitsOperational(t *testing.T) {
	...
	// Default report: all 4 content-gap codes present, zero-filled, in order.
	wantOrder := []string{"missing_entity", "missing_field", "ambiguous_entity", "conflicting_kb_data"}
...
// TestKBGapReportFor_ExcludesSimulatorChannel proves prompt-testing traffic
// never inflates the operator-facing report: the simulator drives drafts
// through the exact same response.Service pipeline as a real conversation
// (see kbGapFilterClause's own doc comment), so without this exclusion a
// test send would show up identically to a real customer escalation.
func TestKBGapReportFor_ExcludesSimulatorChannel(t *testing.T) {
	st := dbtest.New(t)
	ctx := context.Background()
	orgID, _, chatID := seedOneChat(t, st)

	seedGapEvent(t, st, chatID, "missing_field") // channel: whatsapp
	if _, err := st.WriteDraftSet(ctx, "simulator", chatID, uuid.NullUUID{}, []store.DraftOption{
		{Ordinal: 1, Text: "x", ReplyLanguage: "ru", Escalate: true, KBGapReasonCode: "missing_field"},
	}); err != nil {
	...
	if got := countFor(report.Counts, "missing_field"); got != 1 {
		t.Errorf("missing_field count = %d, want 1 (only the whatsapp event — the simulator one must be excluded)", got)
	}
	if len(report.Recent) != 1 || report.Recent[0].Channel != "whatsapp" {
		t.Errorf("Recent = %+v, want exactly the whatsapp event", report.Recent)
	}
}

// TestKBGapReportFor_TopTargetEntitiesRanksByCount proves the report answers
// "which product/tariff/etc. causes the most escalations" — the question a
// reason-code count alone (or manually scanning a bounded Recent sample)
// cannot answer once matching events exceed that sample.
func TestKBGapReportFor_TopTargetEntitiesRanksByCount(t *testing.T) {
	st := dbtest.New(t)
	ctx := context.Background()
	orgID, _, chatID := seedOneChat(t, st)

	seedGapEventWithTarget(t, st, chatID, "product", "coffee-machine", nil)
	seedGapEventWithTarget(t, st, chatID, "product", "coffee-machine", nil)
	seedGapEventWithTarget(t, st, chatID, "product", "coffee-machine", nil)
	seedGapEventWithTarget(t, st, chatID, "product", "blender", nil)
	seedGapEvent(t, st, chatID, "unsupported_request") // no target at all

	report, err := st.KBGapReportFor(ctx, store.KBGapFilter{OrgID: orgID})
	if err != nil {
		t.Fatalf("KBGapReportFor: %v", err)
	}
	if len(report.TopTargetEntities) != 2 {
		t.Fatalf("TopTargetEntities = %+v, want exactly 2 (the no-target event must be excluded)", report.TopTargetEntities)
	}
	if got := entityCountFor(report.TopTargetEntities, "product", "coffee-machine"); got != 3 {
		t.Errorf("coffee-machine count = %d, want 3", got)
	}
	if got := entityCountFor(report.TopTargetEntities, "product", "blender"); got != 1 {
		t.Errorf("blender count = %d, want 1", got)
	}
	if report.TopTargetEntities[0].TargetEntityRef != "coffee-machine" {
		t.Errorf("TopTargetEntities[0] = %+v, want coffee-machine first (highest count)", report.TopTargetEntities[0])
	}
}

// TestKBGapReportFor_TopMissingFieldsRanksByCount proves the field-name
// rollup counts by EVENT, not by how many times a field is listed within
// one event's own missing_fields (that would double count — see the P1
// dedupe/idempotent-insert fix), and stays scoped by entity type.
func TestKBGapReportFor_TopMissingFieldsRanksByCount(t *testing.T) {
	st := dbtest.New(t)
	ctx := context.Background()
	orgID, _, chatID := seedOneChat(t, st)

	seedGapEventWithTarget(t, st, chatID, "product", "coffee-machine", []string{"price", "price"})
	seedGapEventWithTarget(t, st, chatID, "product", "blender", []string{"price", "warranty_terms"})
	seedGapEventWithTarget(t, st, chatID, "tariff", "basic", []string{"fee"})

	report, err := st.KBGapReportFor(ctx, store.KBGapFilter{OrgID: orgID})
	if err != nil {
		t.Fatalf("KBGapReportFor: %v", err)
	}
	if got := fieldCountFor(report.TopMissingFields, "product", "price"); got != 2 {
		t.Errorf("product/price count = %d, want 2 (one per event, not per repetition)", got)
	}
	if got := fieldCountFor(report.TopMissingFields, "product", "warranty_terms"); got != 1 {
		t.Errorf("product/warranty_terms count = %d, want 1", got)
	}
	if got := fieldCountFor(report.TopMissingFields, "tariff", "fee"); got != 1 {
		t.Errorf("tariff/fee count = %d, want 1", got)
	}
	if report.TopMissingFields[0].FieldName != "price" || report.TopMissingFields[0].Count != 2 {
		t.Errorf("TopMissingFields[0] = %+v, want price/2 first (highest count)", report.TopMissingFields[0])
	}
}
...
// TestKBGapReportFor_LimitIsClampedToMaximum is the P2 finding's proof: GET
// /kb/gaps describes Recent as a bounded page, so an oversized caller value
// (here, larger than the actual number of matching rows in the table) must
// still come back capped at the documented maximum (500 — see
// maxKBGapRecentLimit) rather than however many rows actually match. 505
// options in ONE WriteDraftSet call (one transaction) keeps this fast.
func TestKBGapReportFor_LimitIsClampedToMaximum(t *testing.T) {
	st := dbtest.New(t)
	ctx := context.Background()
	orgID, _, chatID := seedOneChat(t, st)

	const seeded = 505
	opts := make([]store.DraftOption, seeded)
	for i := range opts {
		opts[i] = store.DraftOption{Ordinal: i + 1, Text: "x", ReplyLanguage: "ru", Escalate: true, KBGapReasonCode: "missing_field"}
	}
	if _, err := st.WriteDraftSet(ctx, "whatsapp", chatID, uuid.NullUUID{}, opts); err != nil {
		t.Fatalf("WriteDraftSet: %v", err)
	}

	report, err := st.KBGapReportFor(ctx, store.KBGapFilter{OrgID: orgID, Limit: 2147483647})
	if err != nil {
		t.Fatalf("KBGapReportFor: %v", err)
	}
	if len(report.Recent) != 500 {
		t.Fatalf("Recent = %d events (out of %d seeded), want clamped to 500", len(report.Recent), seeded)
	}
}
