-- xchats SQLite schema, part 13: the customer-management (CRM) layer.
...
-- JID, tg_contacts by telegram user id, channel_contacts by the provider's own
-- IGSID/PSID/E.164), each scoped to one account, so
...
-- every channel's tables, and what makes a future channel reach the CRM layer
-- with no migration at all (see crm_customer_identities.channel below).
--
-- FK-less columns: crm_customer_identities.account_id/contact_id and
-- crm_followups.conversation_id can each reference a wa_*, tg_* OR channel_*
-- row, and a single foreign key cannot express an either/or reference — the same
...
--
-- channel deliberately carries NO CHECK constraint, matching
-- channel_accounts.channel in 0011_channel_core: that migration's header
-- spells out the cost of a closed channel vocabulary (SQLite has no
-- ALTER...ALTER CHECK, only a full table rebuild), and this table would have
-- needed widening for instagram/messenger/whatsapp_cloud the moment it
-- landed. A future channel reaches the CRM layer with no migration at all.
CREATE TABLE crm_customer_identities (
    id               TEXT PRIMARY KEY NOT NULL DEFAULT (lower(hex(randomblob(4)) || '-' || hex(randomblob(2)) || '-4' || substr(hex(randomblob(2)),2) || '-' || substr('89ab',abs(random()) % 4 + 1,1) || substr(hex(randomblob(2)),2) || '-' || hex(randomblob(6)))),
    organization_id  TEXT NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    customer_id      TEXT NOT NULL REFERENCES crm_customers(id) ON DELETE CASCADE,
    channel          TEXT NOT NULL,
    account_id       TEXT NOT NULL, -- wa_accounts | tg_accounts | channel_accounts; no FK — see file header
    contact_id       TEXT NOT NULL, -- wa_contacts | tg_contacts | channel_contacts; no FK — see file header
...
CREATE TABLE crm_followups (
    ...
    organization_id     TEXT NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    customer_id         TEXT NOT NULL REFERENCES crm_customers(id) ON DELETE CASCADE,
    conversation_id     TEXT,          -- wa_chats | tg_chats | channel_chats; no FK — see file header. NULL when created from the customer page
