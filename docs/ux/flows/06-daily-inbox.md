...
> **Verified:** 2026-08-30 against the chatboard composition, inbox and CRM stores, chat list/thread/composer, customer panel, assistant panel, and new-message dialog.
>
> **Status:** Living implementation roadmap. Stable `INB-*` IDs identify remaining work. `P0` can show or send one customer’s data in another customer’s context; `P1` can lose or falsely report customer communication; `P2` is frequent workflow friction; `P3` is refinement.
>
> **Purpose:** Trace the operator journey for daily message triage on `/chatboard`: scanning conversation lists, selecting active threads, inspecting customer context, reviewing and editing AI draft suggestions, replying via manual input or AI approval, and composing new outbound conversations.
...
flowchart TD
    ...
    RightPanelCheck -->|Initial default: Customer; selection persists across chats| CustomerTabState
    ...
    TriageDecision -->|Clicks Dismiss| DismissState[Draft cards cleared locally; they can return after reload or reselect]
    DismissState --> EmptyDraftState
...
### INB-01 — [P2] AI Draft Availability Is Hidden Behind an Unbadged Tab
...
### INB-02 — [P2] Rigid 3-Column Layout Compresses Message Thread on Laptops
...
### INB-03 — [P2] No Keyboard Shortcut to Approve and Send AI Drafts
...
### INB-04 — [P1] "Resolve" Button in Thread Header Is Non-Functional
...
### INB-05 — [P2] Stale AI Drafts Disappear Silently Without Feedback
...
### INB-06 — [P3] Redundant "New Message" Triggers with No Keyboard Shortcut
...
### INB-07 — [P2] Smile (Emoji) Button in Composer Is an Unwired Placeholder
...
### INB-08 — [P0] Rapid Chat Switching Can Render the Previous Chat’s Data
...
**Suggested change:** Capture the requested chat/customer ID, cancel obsolete work with `AbortController` where supported, and apply a response only if it still matches the active selection. Keep request identity and mutation guards in the Pinia stores rather than duplicating them in each component.

**Acceptance criteria:**

- Selecting A and then B can never render A’s messages, drafts, profile, or timeline under B.
- Obsolete requests cannot clear B’s loading/error state.
- Search/filter list requests follow the same latest-request-wins rule.
- Tests force responses to finish out of order.

---

### INB-09 — [P1] Manual Send Failures Are Invisible and Attachments Are Lost
...
### INB-10 — [P2] Green “Online” Dot Is Always Shown
...
### INB-11 — [P1] Inbox and Message History Are Silently Truncated
...
### INB-12 — [P2] “New Message” Supports Only QR WhatsApp/Simulator Accounts
...
### INB-13 — [P0] Attachment Upload Can Send to the Wrong Customer

**What happens today:** `send()` checks that `activeId` exists, uploads files, and only then builds `/chats/${this.activeId}/messages`. The chat list remains interactive during upload. If the operator switches from chat A to chat B while files upload, the eventual request uses B’s id with A’s text and attachments.

**Suggested change:** Capture an immutable `chatId` at send start and use it for the complete operation. Associate sending/error state with that chat instead of one global boolean. Switching chats may remain possible, but it must never retarget an in-flight operation.

**Acceptance criteria:**

- Every send is bound to the chat active when Send was pressed.
- Switching chats during upload cannot change the destination.
- The original chat receives success/error state even when it is no longer selected.
- Double-send is prevented per chat without freezing unrelated conversations.
- A test pauses upload, switches chats, resumes, and verifies the original chat id.

**Primary ownership:** `stores/inbox.ts`; `Composer.vue` and `ChatThread.vue` only render store state and emit intent.

---

### INB-14 — [P2] Dismissed AI Drafts Return

**What happens today:** **Dismiss** assigns `inbox.drafts = []` locally. It does not call a backend dismiss/reject endpoint, so the same options return when drafts are refetched or the chat is reselected.

**Suggested change:** Add an explicit store action and backend state transition for dismissing the current draft set. If dismissal is intentionally local, rename it to **Hide for now** and communicate that behavior.

**Acceptance criteria:**

- The label matches persistence semantics.
- Permanent dismissal survives reselect and refresh.
- The component never mutates Pinia state directly; it calls an explicit action.

---

### INB-15 — [P2] Loading the Inbox Briefly Shows a False Empty State

**What happens today:** `ChatList.vue` renders “No chats yet” whenever the array is empty, including while `loadingChats` is true during initial load or filter changes.

**Suggested change:** Render loading, failed, filtered-empty, and truly-empty states separately.

**Acceptance criteria:**

- Initial load shows a stable skeleton or loading state.
- Request failure shows Retry rather than an empty inbox.
- Empty copy reflects active search/filter state.

---

### INB-16 — [P2] Active Conversation Is Not Represented in the URL

**What happens today:** Every selected conversation remains at `/chatboard`. Refresh loses the selection, and customer/follow-up navigation relies on in-memory store mutations after routing.

**Suggested change:** Represent the active chat as a route parameter or query value. Restore it on load, preserve filters where practical, and show a clear not-found state for inaccessible chats.

**Acceptance criteria:**

- A selected chat can be refreshed, bookmarked, and shared with another authorized user.
- Browser Back/Forward restores conversation selection.
- Authorization remains enforced by the API.

---

## Implementation Order

1. `INB-13` and `INB-08` — eliminate cross-customer data and send races.
2. `INB-09` — make send success/failure trustworthy and preserve attachments.
3. `INB-11` — expose complete chat and message history.
4. `INB-04` — remove or implement the false Resolve action.
5. `INB-14`–`INB-16` — make dismissal, loading, and navigation state honest.
6. `INB-01`–`INB-03`, `INB-05`–`INB-07`, `INB-10`, and `INB-12` — improve daily speed and clarity.
