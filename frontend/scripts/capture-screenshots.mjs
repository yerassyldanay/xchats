...
//   2. `make seed-demo` already run against that instance's database, so
...
// against either a fresh `make seed-demo` or an already-used dev database —
...
// throughout (see `make seed-demo` / backend/internal/store/seed_demo_data.go
...
const SHOTS = [
  {
    ...
    prepare: async (page) => {
      ...
      await page.getByRole('button', { name: /Aigerim Bekova/ }).first().click()
  ...
  },
  {
    ...
    path: '/campaigns',
    prepare: async (page) => {
      await page.getByText('Рассылка', { exact: false }).first().waitFor({ timeout: 15_000 })
    },
  },
]
...
async function main() {
  ...
  await browser.close()
  log(`done — ${SHOTS.length} screenshots written to docs/images/ and frontend/public/screenshots/`)
}
