...
import { costLabel, execIsPass, type TestCaseGroup } from '@/lib/evalMatrix'
...
// doctrine: matrix first, cards only when a user chooses to inspect one).
...
defineProps<{ group: TestCaseGroup; runId: string; expanded: boolean }>()
...
function toggleChecks(i: number) {
...
<template>
  <div class="rounded-xl border border-border bg-card overflow-hidden">
    ...
    <div v-if="expanded" class="border-t border-border">
      <div class="px-4 py-3 bg-muted/30 space-y-2">
      ...
      <div class="divide-y divide-border">
        <div v-for="(e, i) in group.execs" :key="i" class="p-4 space-y-3">
          ...
          <img
            ...
            :src="evalsApi.inputImageURL(runId, e.subject.case_id)"
          ...
          <div class="grid gap-3 sm:grid-cols-2">
            ...
            <template v-if="e.family === 'extract'">
              <div class="space-y-1">
                <div class="text-xs font-medium text-muted-foreground">Промпт</div>
                <p class="text-sm font-mono bg-muted/50 rounded-lg p-2.5">{{ e.variant.prompt?.name }}@v{{ e.variant.prompt?.version || '?' }}</p>
              </div>
              <div class="space-y-1">
          ...
          <div>
            <button class="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition select-none" @click="toggleChecks(i)">
              <component :is="expandedChecks.has(i) ? ChevronDown : ChevronRight" class="w-3.5 h-3.5" />
              проверки ({{ (e.scores ?? []).filter((s) => s.status === 'pass').length }}/{{ (e.scores ?? []).length }})
            </button>
            <div v-if="expandedChecks.has(i)" class="mt-2 space-y-2">
              <p v-if="!e.scores?.length" class="text-xs text-muted-foreground italic">Проверки не выполнялись (ответ не разобран).</p>
              ...
              <div v-if="e.family === 'extract' && e.extract" class="grid grid-cols-2 gap-x-4 gap-y-1 text-[11px] text-muted-foreground bg-muted/40 rounded-lg p-2.5">
                <span v-if="e.extract.content_kind"><b class="text-foreground">Тип:</b> {{ e.extract.content_kind }}</span>
                <span v-if="e.extract.language"><b class="text-foreground">Язык:</b> {{ e.extract.language }}</span>
                <span v-if="e.extract.visibility_suggestion"><b class="text-foreground">Видимость:</b> {{ e.extract.visibility_suggestion }}</span>
                <span v-if="e.extract.media_role_hint"><b class="text-foreground">Роль медиа:</b> {{ e.extract.media_role_hint }}</span>
              ...
              <button class="inline-flex items-center gap-1 text-xs text-primary hover:underline" @click="rawJsonExec = e">
