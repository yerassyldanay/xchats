...
export default {
  ...
  landing: {
    ...
    hero: {
      ...
      titlePrefix: 'Self-hosted омниканальный инбокс с',
      titleHighlight: 'AI без галлюцинаций',
      titleSuffix: '',
      subtitle:
      ...
      screenshotAlt: 'Инбокс xchats с черновиком ответа от ИИ, ожидающим одобрения',
      quickstartHint: 'Затем откройте http://localhost:8081 — смотрите визуальный тур ниже.',
    },
    platforms: {
      eyebrow: 'ПЛАТФОРМЫ',
    ...
    showcase: {
      ...
      title: 'Один продукт — пять настоящих экранов',
      description: 'Каждая панель ниже — реальный скриншот с работающего инстанса, заполненного командой make seed-demo, а не макет.',
      ...
      crmTab: 'Мини-CRM и задачи',
      crmDesc: 'Лёгкий профиль клиента и доска задач, сгруппированных по времени: просрочено, сегодня, завтра, позже.',
      crmAlt: 'Доска задач, сгруппированных на просроченные, сегодняшние, завтрашние и более поздние',
      mcpTab: 'MCP и симулятор',
      mcpDesc: 'Проверьте ассистента без риска в симуляторе — или позвольте ChatGPT и Claude готовить изменения базы знаний через MCP.',
      mcpAlt: 'Симулятор, тестирующий ассистента без подключения реального канала',
    },
    architecture: {
      eyebrow: 'АРХИТЕКТУРА',
      title: 'Осознанно скучная инфраструктура',
      description: 'Не нужен кластер, нет привязки к вендору, нет модели, которой вы обязаны доверять.',
      binaryTitle: 'Один бинарник Go + встроенный SQLite',
      binaryDesc: 'Не нужен ни Postgres, ни Redis, ни управляемая база данных. Бэкап — это копия одного файла.',
      privacyTitle: 'Приватность по умолчанию, self-hosted',
      privacyDesc: 'Работает на вашем сервере или ноутбуке. Данные клиентов не покидают инфраструктуру, которую контролируете вы.',
      modelTitle: 'Любая модель',
      modelDesc: 'OpenAI, Claude, Gemini, OpenRouter или локальная модель через Ollama — провайдер меняется в настройках, а не в коде.',
    },
    how: {
