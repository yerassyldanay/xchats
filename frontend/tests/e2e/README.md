...
pages (Конструктор `/playground`, Редактор `/knowledge-base`).
...
- Tests share one backend draft and run serially (`fullyParallel: false`); each
  guards for "draft open or not" so they're rerunnable.
