...
`sendText` and `sendMedia`** (one file per call; a `text` + N `media_ids` send fans out to N+1 messages —
see `TODO.md` B7). The human/AI send pipeline (`POST /xchats/api/v1/chats/{id}/messages` →
`sendText`/`sendMedia` → store `key.id` → status via `messages.update`) is in `4-wa-connection-example.md`.
`sendWhatsAppAudio` / `sendSticker` / `sendReaction` come after. They share the same pipeline — only the
action + body differ.
