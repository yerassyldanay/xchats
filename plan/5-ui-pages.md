...
- **Left nav rail** — Inbox/Chats (the only live destination in v1); Contacts, WhatsApp
  accounts, AI assistant, Settings appear as **deferred placeholders**; the current org + user at
  the bottom.
- **Chat list (pane 1)** — search; filter tabs (unassigned / mine / all, by status);
  each row shows avatar, contact name, last-message preview, time, unread badge, and which
  WhatsApp account it came in on. Live-updates over SSE (`chat.*`, `message.created`).
...
- **Right panel (pane 3)** — v1 has **two sections only** (the reference screenshot's Auto-responder
  ...
  - **AI draft card** — the suggested reply for the open chat. A **"Suggest reply"** button
...
attributes, and the contact's chats.
...
assets**; a **playground** to dry-run a draft against a sample chat; publish / version the
config. All stored in Postgres.

### Settings (deferred)

- **Organization** — auto-response mode + time; org profile.
- **Users** — list users; **add user** (email + password); they join the default org.
